...
Nhập tên client và địa chỉ server (ví dụ: localhost:5000)
Nhấn "Connect" - hệ thống sẽ tự động sinh key mới
Bước 2: Upload file
Chọn file nhạc cần upload
Nhấn "Upload File"
Hệ thống sẽ:
...
Xem danh sách file pending (auto-refresh mỗi 3 giây)
Approve hoặc Reject file
Download file đã được approve
Bước 4: Quản lý khóa
...
Lỗi thường gặp
"Plaintext is too long": RSA key size không đủ (đã fix bằng hybrid encryption)
"Connection timeout": Server không chạy
"Key loading failed": File key bị hỏng
📊 Monitoring
